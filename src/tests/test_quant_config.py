"""Tests for QuantConfig dataclass and OpQuantConfig.from_descriptor()."""
import pytest
import torch
...
class TestQuantConfigDefaults:
    def test_default_values(self):
        ...
        assert cfg.prescale_granularity == "per_tensor"
...
class TestToOpConfigBasic:
    def test_basic_int8_per_tensor(self):
        ...
        # Input/output schemes (same as weight when a_format is None)
    ...
    def test_with_a_format_mixed_precision(self):
        ...
        assert result.output.format.name == "fp8_e4m3"
    ...
    def test_a_format_none_uses_weight_format(self):
        ...
        assert result.output.format.name == "nf4"

    def test_scale_storage_pot(self):
        """scale_storage='pot' → scale_format='pot' on QuantScheme."""
        cfg = QuantConfig(w_format="int8", scale_storage="pot")
        result = cfg.to_op_config()
        assert result.weight.scale_format == "pot"
        assert result.input.scale_format == "pot"
        assert result.output.scale_format == "pot"
...
class TestToOpConfigTransform:
    def test_transform_none(self):
        ...
        assert isinstance(result.output.transform, IdentityTransform)
    ...
    def test_transform_hadamard(self):
        ...
        assert isinstance(result.output.transform, HadamardTransform)
    ...
    def test_transform_smoothquant(self):
        ...
        assert isinstance(result.output.transform, SmoothQuantTransform)
...
class TestQuantConfigFieldStorage:
    ...
    def test_prescale_granularity_default_follows_a_granularity(self):
        """prescale_granularity defaults to a_granularity when not set."""
        cfg = QuantConfig(a_granularity="per_channel")
        assert cfg.prescale_granularity == "per_channel"


# ===========================================================================
# OpQuantConfig.from_descriptor()
# ===========================================================================

class TestFromDescriptor:
    def test_basic(self):
        desc = {"format": "int8", "granularity": "per_tensor"}
        result = OpQuantConfig.from_descriptor(desc)
        assert isinstance(result, OpQuantConfig)
        assert result.input is not None
        assert result.weight is not None
        assert result.output is not None
    ...
    def test_weight_only_with_act_format_raises(self):
        ...
        with pytest.raises(ValueError, match="act_format.*weight_only"):
            OpQuantConfig.from_descriptor(desc)


# ===========================================================================
# OpQuantConfig.from_descriptor() — error paths
# ===========================================================================

class TestFromDescriptorErrors:
