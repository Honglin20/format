...
> **Note**: `ExperimentRunner` is a high-level batch wrapper around `QuantSession` — it parameterizes `QuantSession` with each config, runs calibrate/analyze/evaluate, and aggregates results.
...
| `observers` | `List[Observer]` 或 None | 否 | 挂载到 `AnalysisContext` 的 Observer 列表。`None` 使用默认 `[QSNRObserver()]`；空列表 `[]` 表示不挂载任何 Observer。 |
| `default_calibrator` | `ScaleStrategy` | 否 | 各配置的默认校准策略。可通过 study part 覆盖。默认 `MSEScaleStrategy()`。 |

> **Note**: `MSEScaleStrategy()` differs from `QuantSession`'s default `MaxScaleStrategy()`. This is intentional: research experiments benefit from MSE-optimal scales; `QuantSession`'s `MaxScaleStrategy` is the conservative default for general use.
...
with session.calibrate():
    for input_tensor in calib_data:
        eval_fn(session, input_tensor)        # 前向传播 → forward hook 收集激活 amax
    # exit: 自动计算并回写 _output_scale 到各模块
```

校准完成后，`eval_fn` 的副作用（`_output_scale` 写入）已持久化到 `session.qmodel`。
...
with session.analyze(observers=observers) as ctx:
    for input_tensor in analyze_data:
        eval_fn(session, input_tensor)        # 前向传播 → Observer 在各量化关键点记录 fp32/quant 对
report = ctx.report()                # 聚合所有 Observer 数据 → Report 对象
```

此阶段通过 `eval_fn` 驱动前向传播，Observer 在各量化关键点被动收集数据。
...
| `calib_data=None` 且 `analyze_data=None` 且 `eval_data=None` | 返回不含分析/评估字段的条目（`report`、`qsnr_per_layer`、`mse_per_layer`、`fp32`、`quant`、`delta` 均不存在） | 仅构建 session，跳过所有阶段 |
| `eval_data=None` | `fp32` / `quant` / `delta` 字段为 `None` | 不影响 calibrate 和 analyze 阶段 |
| `analyze_data=None` | `report`、`qsnr_per_layer`、`mse_per_layer` 字段不存在于返回值中 | 不影响 calibrate 和 evaluate 阶段 |
