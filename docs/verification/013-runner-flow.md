...
session.use_fp32()
fp32_metrics_list = []
for input_tensor, label in eval_data:
    output = session(input_tensor)
    fp32_metrics_list.append(eval_fn(output, label))
...
for input_tensor, label in eval_data:
    output = session(input_tensor)
    quant_metrics_list.append(eval_fn(output, label))

# aggregate metrics (mean of all batches)
fp32_metrics = {k: mean([m[k] for m in fp32_metrics_list]) for k in fp32_metrics_list[0]}
quant_metrics = {k: mean([m[k] for m in quant_metrics_list]) for k in quant_metrics_list[0]}
...
```python
{
    "MXINT-8": {
        ...
        "qsnr_per_layer": {                                # 从 report 提取的 QSNR 摘要（层名 → dB）
            "0.linear": 42.99,
            ...
        },
        "mse_per_layer": {                                 # 从 report 提取的 MSE 摘要（层名 → 标量）
            "0.linear": 0.0012,
            ...
        },
    },
...
| `analyze_data=None` | `report` 字段为 `None` | 不影响 calibrate 和 evaluate 阶段 |
...
- 每个条目含 `fp32`, `quant`, `delta`, `report`, `qsnr_per_layer`, `mse_per_layer`
...
- `qsnr_per_layer` 和 `mse_per_layer` 是从 `report` 提取的摘要
