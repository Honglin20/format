...
**P1~P5（已完成 ✅）**：Transform 体系（SmoothQuant/Hadamard/PreScale）、Calibration 管线（4 种策略 + pipeline）、NF4/LookupFormat、参数化格式注册、LSQ 可学习量化（ADR-006）

**P6（当前）**：Coarse Model — 无真实部署的延迟/显存估算。公式参考 `docs/architecture/007-p6-cost-model.md`

**P7~P9（未开始）**：自动格式搜索、融合 Kernel、ONNX custom op ORT 推理
...
| `docs/architecture/006-p5-learnable-pre-scale.md` | P5 LSQ：PreScaleTransform + LayerwiseScaleOptimizer（ADR-006）|
| `docs/architecture/007-p6-cost-model.md` | P6 Coarse Model 架构设计（包结构、Session 集成、测试策略）|
| `docs/refs/p6-cost-model-formulas.md` | P6 Coarse Model **公式权威来源**（原理 + 所有计算公式，实现必须与此一致）|
